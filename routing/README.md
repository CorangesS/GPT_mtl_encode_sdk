...
部署与使用、双机测试流程见：**[../docs/需求3_路由管理部署与使用.md](../docs/需求3_路由管理部署与使用.md)**。
...
2. **部署并配置 Web 界面**  
   ...
   - 若用 **nmos-cpp 单独部署**：需部署 [nmos-js](https://github.com/sony/nmos-js) 并配置 Registry Base URL。

3. **自研节点接入**  
   - 运行 `scripts/register_node_example.py` 向 Registry 注册；`--mode receiver`（默认）/`sender`/`both`；`--href` 需为从浏览器可达的节点地址；`--save-config .nmos_node.json` 供 IS-05 读取 receiver_id。  
   - **IS-05 服务端**：运行 `is05_server/app.py`；详见 [is05_server/README.md](is05_server/README.md)。  
   - 自研 Receiver 无法 Connect/Staged/Active 的排查见 [../docs/需求3_路由管理部署与使用.md](../docs/需求3_路由管理部署与使用.md) 第六节。

4. **外购 ST2110 设备**  
   将其 Registry 配置为同一地址，即可在 Controller 中与自研节点一起发现、连接与管理。

**完整流程见 [../docs/需求3_路由管理部署与使用.md](../docs/需求3_路由管理部署与使用.md)。**
