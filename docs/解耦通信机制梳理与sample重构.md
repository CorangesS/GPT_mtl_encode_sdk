...
- `mtl_sdk` 的 **SPSC 无锁 ring** 实际用于：MTL 回调线程触发“帧就绪”时，把一个索引 `idx` 放入 `SpscRing<int> ready`_，让应用线程 `poll()` 非阻塞轮询/取出；
...
1. `release()` 由**调用方（接收侧/应用侧）**触发，而不是由 `encode_sdk` 触发：`Context::VideoRxSession::release(VideoFrame&)` 只是把 buffer index 归还给 MTL RX 的 `free`_ 池，并调用 `st20p_rx_put_frame`。
2. 在 `MtlVideoRx` 中，如果不调用 `release()`，`free`_ 会耗尽（`query_ext_frame` 返回 -1），导致 `poll()` 获取不到新帧；这就是“生命周期契约”的硬约束。
...
下面这组命令用于“IS-05 状态驱动收流 -> 落 YUV -> 离线编码 MP4”，适用于双机场景。

### 8.1 接收端：IS-05 驱动 + 落 YUV（不实时编码）

在 `build/` 目录运行：

```bash
# 先建目录（只需一次）
mkdir -p ../video/receive ../video/decode

./is05_receiver_only_daemon \
  --port kernel:enp6s0 \
  --sip 192.168.1.100 \
  --state ../connection_state.json \
  --yuv-out ../video/receive/recv_is05_run2.yuv
```

说明：

- `--state` 指向 IS-05 服务写入的连接状态文件。
- `--yuv-out` 会生成两个文件：`../video/receive/recv_is05_run1.yuv` 与 `../video/receive/recv_is05_run1.yuv.json`。
- `*.json` 的字段（`pix_fmt/size_y/size_u/size_v/bytes_per_frame/frames_written`）与 `yuv_encode` 直接兼容。

### 8.2 发送端（示例）

发送端按你的实际环境发流即可。示例（30fps）：
...
### 8.3 离线编码：走 yuv_encode（必走此链路）

在接收端 `build/` 目录运行：

```bash
./yuv_encode \
  --input-yuv ../video/receive/recv_is05_run1.yuv \
  --meta ../video/receive/recv_is05_run1.yuv.json \
  --vcodec h264 \
  --container mp4 \
  --progress \
  --output ../video/decode/recv_is05_run1.mp4
```
...
ffprobe -v error -count_frames -select_streams v:0 \
  -show_entries stream=nb_read_frames,nb_frames,duration,avg_frame_rate \
  -of default=nokey=0:noprint_wrappers=1 ../video/decode/recv_is05_run1.mp4

#播放视频
ffplay -autoexit /home/dd/GPT_mtl_encode_sdk/video/decode/recv_is05_run1.mp4
```
...
- 每次测试请使用唯一输出名（例如 `recv_is05_run_<time>.yuv`），避免覆盖旧文件导致统计混淆。
- 最终离线编码帧数以 `*.yuv.json` 中的 `frames_written` 为准。
- 若发送端是 59.94fps 而接收侧长期只有 46~47fps，建议先降到 30fps 再对齐链路。
