...
static void usage(const char* prog) {
  std::cerr << "Usage: " << prog << " [options]\n"
            << "  --store-root <dir>      Store received raw frames as ring slices under <dir> (required)\n"
            << "  --channel-id <id>       Channel name for ring slice store (default: channel_main)\n"
            << "  --session-id <id>       Session id recorded into each slice (default: auto-generated)\n"
            ...
            << "  --progress              Show receive/store progress\n";
}
...
int main(int argc, char** argv) {
  ...
  if (store_root.empty()) {
    std::cerr << "--store-root is required\n";
    usage(argv[0]);
    return 1;
  }
  ...
  auto store = frame_store::RingSliceStore::open(cfg, vf, audio_format ? &*audio_format : nullptr);
  ...
  while (max_frames == 0 || got < max_frames) {
    ...
    if (v_rx->poll(vf_out, 0)) {
      ...
      if (idle_exit_ms > 0 && had_good_video) {
        ...
        if (std::chrono::duration_cast<std::chrono::milliseconds>(now - last_good_video).count() >=
            idle_exit_ms) {
          stopped_idle = true;
          break;
        }
